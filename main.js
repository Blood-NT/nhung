...
function listenToData() {
  ...
  const checkAll = ref(database, "ALL");
  ...
  onValue(checkThietBi, (snapshot) => {
      ...
      quat = data.QUAT
      updateData()
      sendData(dataThietBi)      // checkbtn( den1,den2,den3,den4,quat);
      console.log("Dữ liệu từ Firebase Realtime Database:", den1, den2, den3, den4, quat);
  }, (error) => {
      console.error("Lỗi khi lắng nghe dữ liệu:", error);
  });
  onValue(checkAll, (snapshot) => {
    const data = snapshot.val();
    den1 = data.substring(1, 4);
    den2 = data.substring(5, 8);
    den3 = data.substring(9, 12);
    den4 = data.substring(13, 16);
    quat = data.substring(17, 20);
    nhietdo = data.substring(20, 25);
    doam = data.substring(25);
    checkbtn(den1, den2, den3, den4, quat);
    updateTandH( nhietdo,doam)
    updateData()
    sendData(dataThietBi)  
    console.log("Dữ liệu từ Firebase Realtime Database:", den1, den2, den3, den4, quat);
  }, (error) => {
...
function sendData(THIETBI) {
  ...
  const checkStatus = ref(database, "STATUS");
  set(checkStatus, { DOAM: doam, NHIETDO: nhietdo })
    .then(() => {
  ...
  const checkAll = ref(database, "ALL");
  set(checkAll, 1+den1+2+den2+3+den3+4+den4+5+quat+nhietdo+doam)
    .then(() => {
      console.log("Dữ liệu đã được ghi lên Firebase Realtime Database.");
    })
    .catch((error) => {
